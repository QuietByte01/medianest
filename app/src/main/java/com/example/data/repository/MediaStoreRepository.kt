...
class MediaStoreRepository(private val context: Context) {
    ...
    suspend fun getAudio(
        ...
        if (showHidden) {
            val hiddenFromFileSystem = scanFileSystemHiddenMedia(MediaType.AUDIO, hiddenFolders, showHidden = true)
        ...
        try {
            val cacheList = com.example.MediaNestApp.instance.database.metadataCacheDao().getAllCache()
            if (cacheList.isNotEmpty()) {
                ...
                for (i in audioList.indices) {
                    ...
                    if (cached != null) {
                        audioList[i] = item.copy(
                            title = cached.title?.takeIf { it.isNotBlank() } ?: item.title,
